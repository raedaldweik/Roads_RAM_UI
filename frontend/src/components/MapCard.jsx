...
// EHS palette (mirrors src/index.css tokens).
const EHS_GREEN = '#0e7d3f';
const EHS_GREEN_HI = '#1ba15c';
const TEAL = '#0e7490';
const INK = '#0a1628';
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      (spec.routes || []).forEach((route, i) => {
        ...
        const id = `route-${i}`;
        const color = route.color || EHS_GREEN;
      ...
      (spec.markers || []).forEach((m) => {
        ...
        if (m.category) {
          el.className = 'mc-dot';
          el.style.background = m.color || EHS_GREEN_HI;
        } else {
          el.className = 'mc-pin';
          el.style.color = m.color || EHS_GREEN;
      ...
      <div className="mc-frame">
        ...
        {TOMTOM_KEY && (
          <button
            ...
            style={{ borderColor: trafficOn ? EHS_GREEN : 'rgba(15,23,42,0.12)', color: trafficOn ? EHS_GREEN : INK }}
            title="Toggle live traffic"
          >
            <span className="mc-traffic-dot" style={{ background: trafficOn ? EHS_GREEN : '#94a3b8' }} />
            Traffic
          </button>
        )}
      </div>
