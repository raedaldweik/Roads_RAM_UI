// Pulls render_chart specs out of a RAM query result.
//
// The `render_chart` MCP tool returns a spec object tagged with `kind: "chart"`
// (type, title, data rows, xKey, yKeys, stacked). Depending on how RAM surfaces
...
/** Returns every chart spec found in a normalized RAM response (in order), de-duplicated. */
export function extractChartSpecs(data) {
  if (!data) return [];
  // Prefer the trace tool calls — they carry tool *outputs* (where the chart
  // spec lives). RAM's embedded response.toolCalls often omits them. Use the
